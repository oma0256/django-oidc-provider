...
from oidc_provider_v2.lib.endpoints.introspection import TokenIntrospectionEndpoint
try:
...
from oidc_provider_v2.compat import get_attr_or_callable
from oidc_provider_v2.lib.claims import StandardScopeClaims
from oidc_provider_v2.lib.endpoints.authorize import AuthorizeEndpoint
from oidc_provider_v2.lib.endpoints.token import TokenEndpoint
from oidc_provider_v2.lib.errors import (
    AuthorizeError,
    ClientIdError,
    RedirectUriError,
    TokenError,
...
from oidc_provider_v2.lib.utils.authorize import strip_prompt_login
from oidc_provider_v2.lib.utils.common import (
    redirect,
    get_site_url,
    get_issuer,
    cors_allow_any,
...
from oidc_provider_v2.lib.utils.token import client_id_from_id_token
from oidc_provider_v2.models import (
    Client,
    RSAKey,
    ResponseType)
from oidc_provider_v2 import settings
from oidc_provider_v2 import signals
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if get_attr_or_callable(request.user, 'is_authenticated'):
                ...
                hidden_inputs = render_to_string('oidc_provider_v2/hidden_inputs.html', context)
...
class ProviderInfoView(View):
    def get(self, request, *args, **kwargs):
        ...
        dic['authorization_endpoint'] = site_url + reverse('oidc_provider_v2:authorize')
        dic['token_endpoint'] = site_url + reverse('oidc_provider_v2:token')
        dic['userinfo_endpoint'] = site_url + reverse('oidc_provider_v2:userinfo')
        dic['end_session_endpoint'] = site_url + reverse('oidc_provider_v2:end-session')
        dic['introspection_endpoint'] = site_url + reverse('oidc_provider_v2:token-introspection')

        types_supported = [response_type.value for response_type in ResponseType.objects.all()]
        dic['response_types_supported'] = types_supported

        dic['jwks_uri'] = site_url + reverse('oidc_provider_v2:jwks')
        ...
        if settings.get('OIDC_SESSION_MANAGEMENT_ENABLE'):
            dic['check_session_iframe'] = site_url + reverse('oidc_provider_v2:check-session-iframe')
...
class CheckSessionIframeView(View):
    ...
    def get(self, request, *args, **kwargs):
        return render(request, 'oidc_provider_v2/check_session_iframe.html', kwargs)
