...
from oidc_provider_v2.lib.endpoints.introspection import INTROSPECTION_SCOPE
from oidc_provider_v2.lib.utils.oauth2 import protected_resource_view
from oidc_provider_v2.lib.utils.token import create_code
from oidc_provider_v2.models import Token
from oidc_provider_v2.tests.app.utils import (
    create_fake_user,
    create_fake_client,
    FAKE_CODE_CHALLENGE,
    FAKE_CODE_VERIFIER,
...
from oidc_provider_v2.views import (
    JwksView,
    TokenView,
    userinfo,
)
...
class TokenTestCase(TestCase):
    ...
    def _post_request(self, post_data, extras={}):
        ...
        format.
        """
        url = reverse('oidc_provider_v2:token')
    ...
    def _get_keys(self):
        ...
        """
        request = self.factory.get(reverse('oidc_provider_v2:jwks'))
    ...
    def _get_userinfo(self, access_token):
        url = reverse('oidc_provider_v2:userinfo')
    ...
    @override_settings(OIDC_GRANT_TYPE_PASSWORD_ENABLE=True,
                       AUTHENTICATION_BACKENDS=("oidc_provider_v2.tests.app.utils.TestAuthBackend",))
    ...
    def do_refresh_token_check(self, scope=None):
        ...
        with patch('oidc_provider_v2.lib.utils.token.time.time') as time_func:
    ...
    @override_settings(
        OIDC_IDTOKEN_SUB_GENERATOR='oidc_provider_v2.tests.app.utils.fake_sub_generator')
    ...
    @override_settings(
        OIDC_IDTOKEN_PROCESSING_HOOK='oidc_provider_v2.tests.app.utils.fake_idtoken_processing_hook')
    ...
    @override_settings(
        OIDC_IDTOKEN_PROCESSING_HOOK=(
                'oidc_provider_v2.tests.app.utils.fake_idtoken_processing_hook',
        )
    )
    ...
    @override_settings(
        OIDC_IDTOKEN_PROCESSING_HOOK=[
                'oidc_provider_v2.tests.app.utils.fake_idtoken_processing_hook',
                'oidc_provider_v2.tests.app.utils.fake_idtoken_processing_hook2',
        ]
    )
    ...
    @override_settings(
        OIDC_IDTOKEN_PROCESSING_HOOK=(
                'oidc_provider_v2.tests.app.utils.fake_idtoken_processing_hook3'))
    ...
    @override_settings(
        OIDC_IDTOKEN_PROCESSING_HOOK=(
                'oidc_provider_v2.tests.app.utils.fake_idtoken_processing_hook4'))
    ...
    def test_client_credentials_grant_type(self):
        ...
        response = self.request_client.post(
            reverse('oidc_provider_v2:token-introspection'), data={'token': access_token},
