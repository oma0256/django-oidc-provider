from oidc_provider_v2.lib.errors import RedirectUriError
...
from oidc_provider_v2.views import AuthorizeView
...
class AuthorizeEndpointMixin(object):
    ...
    def _auth_request(self, method, data=None, is_user_authenticated=False):
        if data is None:
            data = {}
        url = reverse('oidc_provider_v2:authorize')
...
class AuthorizationCodeFlowTestCase(TestCase, AuthorizeEndpointMixin):
    ...
    def test_user_consent_skipped(self):
        ...
        request = self.factory.post(reverse('oidc_provider_v2:authorize'),
                                    data=data)
        # Simulate that the user is logged.
    ...
    @patch('oidc_provider_v2.views.django_user_logout')
    ...
    @patch('oidc_provider_v2.views.render')
...
class TestCreateResponseURI(TestCase):
    ...
    @patch('oidc_provider_v2.lib.endpoints.authorize.create_code')
    @patch('oidc_provider_v2.lib.endpoints.authorize.logger.exception')
